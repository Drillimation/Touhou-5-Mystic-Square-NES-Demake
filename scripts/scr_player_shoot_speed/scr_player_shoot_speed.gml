...
function scr_player_shoot_speed(){
	...
	if (j == 0) {
		...
		switch(global.character) {
			...
			case "MIMA":
				for (var i = 0; i < (floor(global.pow / 25) + 1); i += 1) {
						scr_shoot_bullet_player(4,(90 + (twist * sm_nrm)) - (i * nrm),spr_bullet_mima,0,0,0,false,snd_fire);
				}
				break;
			case "YUUKA":
				for (var i = 0; i < (floor(global.pow / 25) + 1); i += 1) {
						scr_shoot_bullet_player(4,(90 + (twist * sm_nrm)) - (i * nrm),spr_bullet_yuuka,0,0,0,false,snd_fire);
				}
				break;
		}
	}
